...
AABSection::AABSection()
{
	...
	{
		ABLOG(Error, TEXT("Failed to load %s"), *AssetPath);
	}
	...
	{
		ABLOG(Error, TEXT("Failed to load %s"), *GateAssetPath);
	}
	...
	for (FName GateSocket : GateSockets)
	{
		...
		NewGateTrigger->OnComponentEndOverlap.AddDynamic(this, &AABSection::OnSectionExitOverlap);
	}
...
// Called when the g0ȣame starts or when spawned
...
void AABSection::OnSectionExitOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	/*
	TArray<FOverlapResult> OverlapResults;
	FCollisionQueryParams CollisionQueryParam(NAME_None, false, this);
	...
	ABLOG(Warning, TEXT("Gate out, result is.."));
	if (!bResult)
	{
		ABLOG(Warning, TEXT("Nothing inside."));
		if (CurrentState == ESectionState::COMPLETE)
		{
			AABSection* OldSection;
			SetState(ESectionState::BLOCK);
			if(AABSection::CompleteSection != nullptr)//if (CompleteSections.Dequeue(OldSection))
			{
				OldSection = AABSection::CompleteSection;//
				OldSection->SetState(ESectionState::DELETE);
				ABLOG(Error, TEXT("A section will be deleted."));
			}
			//ABCHECK(CompleteSections.Enqueue(this));
			AABSection::CompleteSection = this;//
		}
	...
	{
		ABLOG(Warning, TEXT("There's Something inside Section."));
	}
	*/
}
